    ...
    while options:
        for event in pygame.event.get(): 
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if 330+30 > mouse[0] > 330 and 255 + 30 > mouse[1] > 255:
                    level -= 1
                    print(level)
                if 430+30 > mouse[0] > 430 and 255 + 30 > mouse[1] > 255:
                    level += 1
                    print(level)


        screen.fill((0,0,0))
        pygame.draw.rect(screen, blue, minus)
        pygame.draw.rect(screen, blue, plus)
        if play2_button.draw():
            menu = False
            player_2()
        display_text("Number of players",[300,50])
        display_text("+",[440,258])
        display_text("-",[343,258])
        level_text = font.render(f"{level}", False, white)
        screen.blit(level_text,(390, 258))
        pygame.display.update()
        clock.tick(60)

player_2 = False
def player_2():
    global background, menu, state, score, click, score2, remainder, num
    player_2 = True
    while menu:
        for event in pygame.event.get(): 
            if event.type == pygame.QUIT: 
            ...
            if state == 'ON':
                ...
                if score == 3:
                    num = 3
                if num == 1:
                    player1_text = font.render(f"player{num}", False, text_color)
                    screen.blit(player1_text,(400,10))
                if num == 2:
                    player2_text = font.render(f"player{num}", False, text_color)
                    screen.blit(player2_text,(400,10))
                if num == 3:
                    player3_text = font.render(f"player{num}", False, text_color)
                    screen.blit(player3_text,(400,10))
